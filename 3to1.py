...
def build_model():
    ...
    numpy.random.seed(8)
    model=Sequential()
    #model.add(Dense(300,input_shape=(None,3,3),init='uniform',activation='relu'))
...
if __name__ == '__main__':
    ...
    min4=0
    max4=28.1513                         
    data=sio.loadmat('trainset3.mat') 
    trainset=data['trainset']
    data1=sio.loadmat('aimset3.mat')
    aimset=data1['aimset']
    ...
    mkdir('result0827')
    model.save_weights('result0827/vprediction3to1.h5')
    data2=sio.loadmat('valid3.mat')
    validset=data2['valid']    
    prediction=model.predict(validset)
    result_file = os.path.join(os.getcwd(),'result0827')
    result_file = os.path.join(result_file,'200000samplevprediction3to1.txt')
