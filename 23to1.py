...
def build_model():
    ...
    numpy.random.seed(8)
    model=Sequential()
    #model.add(Dense(300,input_shape=(None,23,3),init='uniform',activation='relu'))
    #model.add(Dense(3, input_shape=(3,)))
    #model.add(Embedding(output_dim=(10,3,), input_dim=3, input_length=10))
    
   # model.add(Dropout(0.8))
    model.add(GRU(300,init='lecun_uniform', input_shape=(23,3), return_sequences=True,activation ='sigmoid'))
...
if __name__ == '__main__':
    ...
    min4=0
    max4=28.1513                         
    data=sio.loadmat('trainset23.mat') 
    trainset=data['trainset']
    data1=sio.loadmat('aimset23.mat')
    aimset=data1['aimset']
    ...
    mkdir('result0827')
    model.save_weights('result0827/vprediction23to1.h5')
    data2=sio.loadmat('valid1.mat')
    validset=data2['valid']    
    prediction=model.predict(validset)
 
    numpy.savetxt('result0827/200000samplevprediction23to1.txt', prediction)