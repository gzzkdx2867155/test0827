...
def build_model():
   ...
   # model.add(Dropout(0.8))
    model.add(GRU(300,init='lecun_uniform', input_shape=(8,3), return_sequences=True,activation ='sigmoid'))
...
if __name__ == '__main__':
    ...
    min4=0
    max4=28.1513                         
    data=sio.loadmat('trainset8.mat') 
    trainset=data['trainset']
    data1=sio.loadmat('aimset8.mat')
    aimset=data1['aimset']
    ...
    mkdir('result0827')
    model.save_weights('result0827/vprediction8to1.h5')
    data2=sio.loadmat('valid8.mat')
    validset=data2['valid']    
    prediction=model.predict(validset)
    result_file = os.path.join(os.getcwd(),'result0827')
    result_file = os.path.join(result_file,'200000samplevprediction8to1.txt')
